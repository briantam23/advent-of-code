...
make sure to start by initializing memory to the program's values. 
...
its output is available at address 0, also just like before. 
...
const restoreState = require('./day2Part1').restoreState;
const runProgram = require('./day2Part1').runProgram;


const getOutput = (input, noun, verb) => {
    const restoredState = restoreState(input, noun, verb);
...
const findNounVerb = (input, targetOutput) => {
    let output = 0;
    let noun = 0, verb = 0;
    
    while(output < targetOutput) {
        output = getOutput(input, noun, verb);
        if(output === targetOutput) break;
        else if(output > targetOutput) {
            noun--;
            output = getOutput(input, noun, verb);
            if(output === targetOutput) break;
        }
...
const [noun, verb] = findNounVerb(input, targetOutput);
