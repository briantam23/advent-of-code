...
increase by the number of values in the instruction after the instruction finishes. 
...
For each test, it will run an output instruction indicating how far the result of the test was 
from the expected value, where 0 means the test was successful. 
...
const runProgram = input => {
    ...
    while(i < input.length) {
        let opCode = input[i].slice(input[i].length - 2);
        if(opCode.length < 2) opCode = '0' + opCode;

        let param1Idx = input[i + 1];
        let param2Idx = input[i + 2];
        let param3Idx = input[i + 3];

        // set mode1 = 0 if mode of the 1st param doesn't exist
        let mode1 = input[i].slice(input[i].length - 3, input[i].length - 2) || '0';
        // set mode2 = 0 if mode of the 2nd param doesn't exist
        let mode2 = input[i].slice(input[i].length - 4, input[i].length - 3) || '0';

        // set value based on either position or relative mode
        let param1, param2;
        mode1 === '0' ? param1 = input[param1Idx] : param1 = param1Idx;
        mode2 === '0' ? param2 = input[param2Idx] : param2 = param2Idx;
        
        if(opCode === '01') input[param3Idx] = String(parseInt(param1) + parseInt(param2));
        else if(opCode === '02') input[param3Idx] = param1 * param2;
        else if(opCode === '03') input[param1Idx] = 1;
        else if(opCode === '04') console.log('Output: ', input[param1Idx]);
        else if(opCode === '99') return;

        parseInt(opCode) < 3 ? i += 4 : i += 2;
    }
}
